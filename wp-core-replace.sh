...
if command -v wp &> /dev/null; then
    ...
    print_warning "WP-CLI chưa cài đặt (một số tính năng sẽ bị giới hạn)"
fi
...
if [ "$HAS_WPCLI" = true ]; then
    ...
    # Đọc version từ wp-includes/version.php nếu không có WP-CLI
    if [ -f "wp-includes/version.php" ]; then
        CURRENT_VERSION=$(grep "wp_version = " wp-includes/version.php | cut -d\' -f2)
    else
...
# Checksum core hiện tại (chỉ khi có WP-CLI)
if [ "$HAS_WPCLI" = true ]; then
    ...
    print_info "Bỏ qua kiểm tra checksum (cần WP-CLI)"
fi
echo ""
...
if [ "$HAS_WPCLI" = true ]; then
    ...
    echo -e "${CYAN}╔════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║${NC}  ${YELLOW}💾 Backup Database${NC}                                         ${CYAN}║${NC}"
    echo -e "${CYAN}╠════════════════════════════════════════════════════════════╣${NC}"
    ...
    if [ -z "$db_backup_choice" ] || [ "$db_backup_choice" == "yes" ]; then
        ...
        print_warning "Đã bỏ qua backup database"
    fi
    echo ""
else
    echo ""
...
if [ -z "$backup_choice" ] || [ "$backup_choice" == "yes" ]; then
    ...
    print_warning "Đã bỏ qua backup core"
fi
echo ""
...
if [ "$HAS_WPCLI" = true ]; then
    ...
    echo -e "${CYAN}║${NC}  2. Truy cập /wp-admin để cập nhật database nếu cần        ${CYAN}║${NC}"
    echo -e "${CYAN}║${NC}  3. Xóa cache nếu có                                       ${CYAN}║${NC}"
fi
